...
bool ADS1263::BeginCustom(uint8_t gain)
{
  ...
  InitialInterfaceSettings.ReportStatus = false;
	...
	InitialMode0Register.RunPulse = false;
	...
	InitialMode1Register.Filter = FilterValue::SINC4;
	InitialMode1Register.SensorBias = SensorBiasValue::NOBIAS;
	SetMode1(&InitialMode1Register);
	
	Mode2Register InitialMode2Register;
  if (gain == 16) {
	  InitialMode2Register.ADC1Gain = ADC1GainValue::V16V;
	  InitialMode2Register.GainBypass = false;
  } else if (gain == 32) {
    InitialMode2Register.ADC1Gain = ADC1GainValue::V32V;
	  InitialMode2Register.GainBypass = false;
  } else {
    InitialMode2Register.ADC1Gain = ADC1GainValue::V1V;
	  InitialMode2Register.GainBypass = true;
  }
	InitialMode2Register.ADC1DataRate = ADC1DataRateValue::SPS38400;
	...
	InitialInputMultiplexerRegister.NegativeChannel = InputMUXValue::AIN1;
...
bool ADS1263::Begin(uint8_t InitADC2)
{
  ...
  InitialInterfaceSettings.ReportStatus = false;
	...
	InitialMode0Register.ChopActive = true;
	InitialMode0Register.IDACRotation = false;
	InitialMode0Register.Delay = DelayValue::NODELAY;
	SetMode0(&InitialMode0Register);
	if (Verbose)
...
// 	return true;
// }
double ADS1263::GetADC1Value()
{
	//int32 max value 2147483647
	double GainAmplitude = (double)(1 << static_cast<uint8_t>(ADC1GainSetting));
	double out = (double)(ADC1Value) * ReferenceVoltage / GainAmplitude / (double)(2147483647);
	if (Verbose)
	{
...
double ADS1263::GetADC1ValueAfterPGA()
{
	return (double)(ADC1Value) * ReferenceVoltage / (double)(2147483647);
}
...
{
	double VoltageRecorded = GetADC1Value();
...
{
	Mode2Register RegisterToSet;
  Serial.println("Entering ADC1Gain Function");
	bool Status = GetMode2(&RegisterToSet);
  Serial.println(Status);
	RegisterToSet.ADC1Gain = ValueToSet;
  Serial.println((uint8_t) ValueToSet);
	return Status & SetMode2(&RegisterToSet);
}
...
bool ADS1263::GetMode2(Mode2Register* RegisterToGet)
{
	...
	uint8_t ReturnByte;
	bool Status = false;
  Serial.println("Middle of GetMode2");
...
bool ADS1263::Send(OpCodesSimple OpCodeToSend)
{
  ...
  // Serial.println("HI4.25");
	digitalWrite(CSSelectPin,LOW);
  // Serial.println("HI4.5");
