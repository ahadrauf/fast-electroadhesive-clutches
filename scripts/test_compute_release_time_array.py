...
t_release_all = []
Fshear_all = []
Ldielectric_all = np.linspace(1, 250, (250 - 1) * 1 + 1)
# Ldielectric_all = np.linspace(1, 2, 2)
start_time = datetime.now()
for idx, L_dielectric in enumerate(Ldielectric_all):
    # start_time = datetime.now()
    print("Starting Ld =", L_dielectric)
    tr = t_release([L_dielectric])
    Fshear = lambda x: calculate_shear_force_pred(w_dielectric=w_pin, L_dielectric=L_dielectric * 1e-3, F_preload=F_preload)

    xopt_all.append(L_dielectric)
...
timestamp += "_V={}".format(V)
data_all = {'xopt_all': xopt_all, 't_release_all': t_release_all, 'Fshear_all': Fshear_all}
