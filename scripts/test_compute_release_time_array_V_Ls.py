...
def calculate_shear_force_pred(w_pin, L_dielectric, F_preload, V):
...
# t_release = lambda x: sim_release_time(x[1] * 1e-3, x[0] * 1e-3, depth_pin, V, t_dielectric, frequency, F_preload, loadcell_start_ratio=0.8)[5]
t_release = lambda x: sim_release_time(L_dielectric, w_pin, depth_pin, x[0], t_dielectric, frequency, F_preload, loadcell_start_ratio=0.8)[5]
...
t_release_all = []
Fshear_all = []
# ws_all = np.linspace(1, 20, (30 - 1) * 4 + 1)  # #np.linspace(2, 10, 3)  # (250 - 1) * 1 + 1)
V_all = np.linspace(100, 500, 3)  # (500 - 100) * 1 + 1)
...
for idx, V in enumerate(V_all):
    ...
    print("Starting V =", V)
    tr = t_release([V])
    Fshear = lambda x: calculate_shear_force_pred(w_pin=w_pin, L_dielectric=L_dielectric, F_preload=F_preload, V=V)
...
timestamp += "_Ld={}_vsV".format(L_dielectric * 1e3)
data_all = {'xopt_all': xopt_all, 't_release_all': t_release_all, 'Fshear_all': Fshear_all, 'V_all': V_all, 'w_pin': w_pin, 'Ld': L_dielectric}
