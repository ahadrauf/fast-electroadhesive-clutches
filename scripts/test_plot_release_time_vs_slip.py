...
if file_name == "":
    ...
    for V, file_names in all_file_names.items():
        ...
        for file_name, speed, V, freq, Fpreload, w_pin in file_names:
            ...
            time_loadcell, load_cell, stage, relay_output, idx_stop_moving, idx_relay_input_rise, idx_relay_input_fall = import_data(file_loc + file_name, zero_load_cell=True,
            ...
            all_data_release_times.append(release_time)
            all_data_slip_pct.append(np.nanmax(slip_dfs))
            print(release_time, np.nanmax(slip_dfs))
...
all_lines = []
print(all_data_release_times)
print(all_data_slip_pct)
scatter = ax2.scatter(all_data_release_times, all_data_slip_pct, s=50, color='k', zorder=99)
...
ax2.grid(True)
ax2.set_xlabel("Release Time (ms)")
