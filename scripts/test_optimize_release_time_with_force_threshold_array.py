...
Fshear_min_all = np.linspace(0, 1, 21)
xopt_all = []
t_release_all = []
Fshear_all = []
for Fshear_min in Fshear_min_all:
    ...
    sol = minimize(fun=t_release, x0=x0, method="COBYLA", bounds=((1., 400.),),
                   constraints={'type': 'ineq', 'fun': Fshear})
    ...
    Fshear_all.append(Fshear(sol.x) + Fshear_min)
    ...
    print(sol)
    # print('--> force:', Fshear(sol.x))
    end_time = datetime.now()
    print("Time Elapsed:", (end_time - start_time).total_seconds(), "seconds")

    x0 = np.array([max(sol.x[0] - 2, 5)])

timestamp += "_wpin={:0.1f}".format(w_pin * 1e3)
data_all = {'Fshear_min_all': Fshear_min_all, 'xopt_all': xopt_all, 't_release_all': t_release_all, 'Fshear_all': Fshear_all}
...
ax.plot(Fshear_min_all, t_release_all)
ax1_right.plot(Fshear_min_all, xopt_all, color='tab:red')

ax.set_xlabel("Desired F_shear (N)")
